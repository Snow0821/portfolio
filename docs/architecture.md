...
기능별 데이터, UI, 레이아웃, 에셋, 스타일과 테스트를 한 변경 경계에서 소유한다. `features/seminars/`는 공통·주제별 논의, 구조화 세미나 원본, 두 HTML 표현과 페이지 초기화 공개 facade를 소유한다. 논의 문서는 런타임 소스가 아니며, 합의된 공통 결정은 `formats/`, 주제 결정은 `data/topics/`로 이동한 뒤 두 표현을 검증한다. 읽기·발표 페이지는 새 탭에 요청한 자료만 표시하고 사이트 헤더, 모드 전환, 이전·다음 버튼과 돌아가기 링크를 두지 않는다. `assets/pdf/`는 수동 검토 후 전달될 네 정적 PDF의 이름과 게시 절차만 예약하며 런타임 생성 서비스는 두지 않는다.
...
- `features/seminars/discussions/topics/<topic-id>.md`에서 목적, 대상, 필수·제외 내용과 열린 질문을 먼저 논의한다.
- 합의한 공통 규칙은 `formats/`, 주제 내용은 `data/topics/`에 반영한다. 논의 문서를 런타임에서 import하지 않는다.
...
- 가까운 세미나 테스트 그룹과 가로·세로 화면 브라우저 검증을 수행하고, 완료 전 `npm run verify`와 작업 이력을 기록한다.
...
- 프로젝트 루트에서 `npm run verify`가 전체 테스트와 공백 검사를 통과한다.
...
- 가로·세로 자료 탭에는 사이트 헤더, 모드·뒤로·돌아가기 링크, 이전·다음 버튼과 슬라이드 카운터가 나타나지 않는다.
- 문서 제목은 현재 주제를 반영하고, 가로 발표 자료의 방향키·Space·스크롤·터치 이동이 정상 동작한다.
- 수동 인쇄 시 가로 슬라이드는 landscape, 세로 읽기 문서는 portrait 출력에 적합한 print CSS를 유지한다.
- PDF 링크를 추가할 때 파일 존재, PDF 시그니처와 실제 브라우저 다운로드를 모두 확인한다.
- 모바일 너비에서 사이트 헤더와 세미나 카드가 사용 가능하고, 두 자료 화면은 잘림이나 불필요한 가로 overflow 없이 콘텐츠에 집중한다.
