...
- 실행 JavaScript, CSS와 테스트 파일은 200줄 이하를 권장한다.
- 200줄을 넘으면 책임 분리 가능성을 검토하고, 300줄을 넘으면 분리하거나 예외 근거를 문서화한다. 일반 모듈화 후속 작업에서 이 기준을 자동 검증에 추가한다.
...
6. 여러 세션에 걸치는 작업은 `docs/status.md`를 최신 작업 목록으로 사용하고 완료된 과정은 이력으로 옮긴다.
